...
contract UniswapWrapperFactoryDeploymentScript is Script {
    ...
    function run() external {
        ...
        uint256 v3Salt = findVanitySalt(
            CREATE2_FACTORY,
            type(UniswapV3WrapperFactory).creationCode,
            abi.encode(evc, nonFungiblePositionManager)
        );
        ...
        uint256 v4Salt = findVanitySalt(
            CREATE2_FACTORY,
            type(UniswapV4WrapperFactory).creationCode,
            abi.encode(evc, v4PositionManager, weth)
        );
        v4Salt = 89803064614613559128942510503412517851878641377354393196606698967848670379431;

        vm.startBroadcast();

        new UniswapV3WrapperFactory{salt: bytes32(v3Salt)}(
            evc,
            nonFungiblePositionManager
        );

        new UniswapV4WrapperFactory{salt: bytes32(v4Salt)}(
            evc,
            v4PositionManager,
            weth
        );
